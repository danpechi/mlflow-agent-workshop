# Databricks notebook source
# MAGIC %md
# MAGIC # Workshop Configuration
# MAGIC
# MAGIC **Single source of truth** for all workshop parameters.
# MAGIC
# MAGIC Every other notebook (`01_setup_data`, `02_evaluate_and_optimize`) starts with
# MAGIC `%run ./00_config` to inherit the values defined here. Edit the widget defaults
# MAGIC below or override at runtime — do **not** hardcode catalog / schema / endpoint
# MAGIC names anywhere else in the workshop.
# MAGIC
# MAGIC **What this notebook does:**
# MAGIC 1. Declares widgets for every configurable name (catalog, schema, volume, LLM endpoint, app name, prompt name, experiment).
# MAGIC 2. Reads any matching environment variables as overrides (so the same notebook works for bundle deploys and interactive use).
# MAGIC 3. Computes all derived paths and fully-qualified names (volume path, prompt registry FQN, table FQNs, file paths, experiment path).
# MAGIC 4. Prints a summary you can verify before running setup or evaluation.
# MAGIC
# MAGIC **Where these values are also referenced:**
# MAGIC - `databricks.yml` — bundle variables (`var.catalog`, `var.schema`, `var.volume`, `var.llm_endpoint`, `var.app_name`)
# MAGIC - `app.yaml` — `LLM_ENDPOINT_NAME`, `PROMPT_REGISTRY_NAME`, `AGENT_PROMPT_VERSION`, `FIXTURES_PATH`
# MAGIC - `agent_server/agent.py` / `prompts.py` / `tools.py` — read the same env vars at runtime

# COMMAND ----------

# MAGIC %md
# MAGIC ## 1. Widget definitions
# MAGIC
# MAGIC Edit the second argument of each `dbutils.widgets.text(...)` call to change the default.

# COMMAND ----------

import os

# Compute short_name from the current user's email (part before @, dots → underscores).
try:
    _email = spark.sql("SELECT current_user()").collect()[0][0]
    _short_name = _email.split("@")[0].replace(".", "_")
except Exception:
    _short_name = os.getenv("USER", "unknown_user").replace(".", "_")

# Helper: prefer an existing widget value, else env var, else fallback default.
def _default(env_var: str, fallback: str) -> str:
    return os.getenv(env_var, fallback)


dbutils.widgets.text("catalog",        _default("WORKSHOP_CATALOG",       "bricks_lab"),                                        "1. UC Catalog")
dbutils.widgets.text("schema",         _default("WORKSHOP_SCHEMA",        "default"),                                           "2. UC Schema")
dbutils.widgets.text("volume",         _default("WORKSHOP_VOLUME",        f"{_short_name}_workshop_data"),                       "3. UC Volume")
dbutils.widgets.text("llm_endpoint",   _default("LLM_ENDPOINT_NAME",      "databricks-claude-sonnet-4-5"),                      "4. LLM serving endpoint")
dbutils.widgets.text("app_name",       _default("WORKSHOP_APP_NAME",      f"{_short_name}-ta"),                          "5. Databricks App name")
dbutils.widgets.text("prompt_name",    _default("WORKSHOP_PROMPT_NAME",   f"{_short_name}_triage_agent_prompt"),                 "6. Prompt registry name (table component)")
dbutils.widgets.text("experiment_name", _default("WORKSHOP_EXPERIMENT_NAME", f"{_short_name}-triage-agent-eval"),        "7. MLflow experiment subdir")
dbutils.widgets.text("alerts_table",   _default("WORKSHOP_ALERTS_TABLE",  f"{_short_name}_sample_alerts"),                       "8. UC table: hand-crafted alerts")
dbutils.widgets.text("eval_table",     _default("WORKSHOP_EVAL_TABLE",    f"{_short_name}_eval_dataset"),                        "9. UC table: eval dataset")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 2. Resolve widget values

# COMMAND ----------

CATALOG          = dbutils.widgets.get("catalog").strip()
SCHEMA           = dbutils.widgets.get("schema").strip()
VOLUME           = dbutils.widgets.get("volume").strip()
LLM_ENDPOINT     = dbutils.widgets.get("llm_endpoint").strip()
APP_NAME         = dbutils.widgets.get("app_name").strip().replace("_", "-")
PROMPT_NAME      = dbutils.widgets.get("prompt_name").strip()
EXPERIMENT_NAME  = dbutils.widgets.get("experiment_name").strip()
ALERTS_TABLE     = dbutils.widgets.get("alerts_table").strip()
EVAL_TABLE       = dbutils.widgets.get("eval_table").strip()

# Validate non-empty
for _name, _val in [
    ("catalog", CATALOG), ("schema", SCHEMA), ("volume", VOLUME),
    ("llm_endpoint", LLM_ENDPOINT), ("app_name", APP_NAME),
    ("prompt_name", PROMPT_NAME), ("experiment_name", EXPERIMENT_NAME),
    ("alerts_table", ALERTS_TABLE), ("eval_table", EVAL_TABLE),
]:
    if not _val:
        raise ValueError(f"Widget '{_name}' is empty. Set it before running downstream notebooks.")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 3. Derived paths and fully-qualified names
# MAGIC
# MAGIC Downstream notebooks reference these variables — never recompute them ad-hoc.

# COMMAND ----------

# Volume + file paths (FUSE-mounted at /Volumes/...)
VOLUME_PATH         = f"/Volumes/{CATALOG}/{SCHEMA}/{VOLUME}"
TOOL_FIXTURES_PATH  = f"{VOLUME_PATH}/tool_fixtures.json"
SAMPLE_ALERTS_PATH  = f"{VOLUME_PATH}/sample_alerts.json"
EVAL_DATASET_PATH   = f"{VOLUME_PATH}/eval_dataset.json"

# Fully-qualified UC names (raw form, e.g. `catalog.schema.table`).
ALERTS_TABLE_FQN    = f"{CATALOG}.{SCHEMA}.{ALERTS_TABLE}"
EVAL_TABLE_FQN      = f"{CATALOG}.{SCHEMA}.{EVAL_TABLE}"
PROMPT_REGISTRY_FQN = f"{CATALOG}.{SCHEMA}.{PROMPT_NAME}"

# Backtick-quoted variants for use in SQL / saveAsTable so names with hyphens
# (e.g. `zscaler-demo`) parse correctly. Use these in spark.sql() / saveAsTable().
def _bt(name: str) -> str:
    return f"`{name}`"

CATALOG_BT          = _bt(CATALOG)
SCHEMA_BT           = _bt(SCHEMA)
ALERTS_TABLE_BT_FQN = f"{_bt(CATALOG)}.{_bt(SCHEMA)}.{_bt(ALERTS_TABLE)}"
EVAL_TABLE_BT_FQN   = f"{_bt(CATALOG)}.{_bt(SCHEMA)}.{_bt(EVAL_TABLE)}"

# MLflow experiment path under the current user's workspace folder
try:
    _username = spark.sql("SELECT current_user()").collect()[0][0]
except Exception:
    _username = os.getenv("USER", "unknown-user")
EXPERIMENT_PATH = f"/Users/{_username}/{EXPERIMENT_NAME}"

# COMMAND ----------

# MAGIC %md
# MAGIC ## 4. Summary

# COMMAND ----------

print("=" * 70)
print("  WORKSHOP CONFIG")
print("=" * 70)
print(f"  Catalog            : {CATALOG}")
print(f"  Schema             : {SCHEMA}")
print(f"  Volume             : {VOLUME}")
print(f"  Volume path        : {VOLUME_PATH}")
print()
print(f"  LLM endpoint       : {LLM_ENDPOINT}")
print(f"  App name           : {APP_NAME}")
print()
print(f"  Prompt registry    : {PROMPT_REGISTRY_FQN}")
print(f"  Alerts table       : {ALERTS_TABLE_FQN}")
print(f"  Eval table         : {EVAL_TABLE_FQN}")
print()
print(f"  Fixtures path      : {TOOL_FIXTURES_PATH}")
print(f"  Sample alerts path : {SAMPLE_ALERTS_PATH}")
print(f"  Eval dataset path  : {EVAL_DATASET_PATH}")
print()
print(f"  MLflow experiment  : {EXPERIMENT_PATH}")
print("=" * 70)